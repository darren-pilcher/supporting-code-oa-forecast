...
Hc.arag = arag_tmp(:,:,ind_tmp);
Hc.t = [arag_t(ind_tmp)];
clear arag*

Hc.pH = pH_tmp(:,:,ind_tmp);
clear pH*
...
for t = 1:length(years)-2
	for m = 1:2
		for e = 1:3
			tmp_arag = ncread(sprintf('%sCFSR_BCOR_%d%s%s_average_arag_bottom5m.nc',RUN_DIR_FC,years(t+2),ens(m),mem(e)),'arag');
			tmp_pH = ncread(sprintf('%sCFSR_BCOR_%d%s%s_average_pH_bottom5m.nc',RUN_DIR_FC,years(t+2),ens(m),mem(e)),'pH');
...
for t = 1:2
	for e = 1:3
		tmp_arag = ncread(sprintf('%sCFSR_BCOR_%d%s%s_average_arag_bottom5m.nc',RUN_DIR_FC,years(t),ens(2),mem(e)),'arag');
                tmp_pH = ncread(sprintf('%sCFSR_BCOR_%d%s%s_average_pH_bottom5m.nc',RUN_DIR_FC,years(t),ens(2),mem(e)),'pH');
...
for e = 1:2
	t = 1;
	tmp_arag = ncread(sprintf('%sCFSR_BCOR_%d%s%s_average_arag_bottom5m.nc',RUN_DIR_FC,years(t),ens(1),mem(e)),'arag');
        tmp_pH = ncread(sprintf('%sCFSR_BCOR_%d%s%s_average_pH_bottom5m.nc',RUN_DIR_FC,years(t),ens(1),mem(e)),'pH');
...
for yr = 1:length(years)
       tmp = load(['./EBS_index_',num2str(years(yr)),'0327.mat']);
      Fc.arag(1,yr,:) = tmp.arag_index; Fc.pH(1,yr,:) = tmp.pH_index; clear tmp

	tmp = load(['./EBS_index_',num2str(years(yr)),'0426.mat']);
       Fc.arag(2,yr,:) = tmp.arag_index; Fc.pH(2,yr,:) = tmp.pH_index; clear tmp
end

% FOR BB version
% for yr = 1:length(years)
%         tmp = load(['./BB_index_',num2str(years(yr)),'0327.mat']);
%         Fc.arag(1,yr,:) = tmp.arag_index; Fc.pH(1,yr,:) = tmp.pH_index; clear tmp
% 
%         tmp = load(['./BB_index_',num2str(years(yr)),'0426.mat']);
%         Fc.arag(2,yr,:) = tmp.arag_index; Fc.pH(2,yr,:) = tmp.pH_index; clear tmp
% end
